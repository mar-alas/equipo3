...
const NOMBRE_ESCENARIO = 'Escenario 97 -- Paso ';
const TWO_SECONDS = 2000;

describe('Escenario 97', () => {
  beforeEach(() => {
    loginPage.visit();
    //cy.screenshot(NOMBRE_ESCENARIO + '0_visit');
  });

  it('Login con exito, creacion y validacion de un post con contenido Email Call to Action, signout', () => {
    ...
    cy.useAleatorioCrearPost().then((formattedPost) => {
      crearPost.crearPostContenidoEmailCallToAction(formattedPost.title, formattedPost.body);
      cy.wait(TWO_SECONDS);
