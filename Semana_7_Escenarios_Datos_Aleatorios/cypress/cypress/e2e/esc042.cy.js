...
const NOMBRE_ESCENARIO = 'Escenario 042 -- Paso ';
const TAKE_SCREENSHOT = false;

describe("Login exitoso, crear pagina con titulo y contenido basico y luego editar el contenido con tags HTML desde API", () => {
  beforeEach(() => {
    loginPage.visit();
    if(TAKE_SCREENSHOT) {
  ...
  it("Login y creación de un page", () => {
    ...
    cy.getHtmlTagAPI().then((data) => {
        // When I edit the page
        crearPagina.editarPagina(initial_title, data.html_tag);
        cy.wait(10000);
        if(TAKE_SCREENSHOT) { 
        ...
        crearPagina.revisarContenidoPagina(initial_title, data.html_tag);
        cy.wait(10000);
        if(TAKE_SCREENSHOT) { 
