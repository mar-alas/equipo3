...
import usePoliDinamicoCrearEmail from './login_helper'

const NOMBRE_ESCENARIO = 'Escenario 21 -- Paso ';
const TWO_SECONDS = 2000;

describe('Escenario 21', () => {
  beforeEach(() => {
    loginPage.visit();
  ...
  it('Login sin datos, login con email no registrado, login con exito', () => {
    ...
    cy.usePoliDinamicoCrearEmail().then((formattedEmail) => {
      // When I enter email using poli-dinamico
      // & I enter password using poli-dinamico
      ...
      loginPage.error(/There is no user with that email address\.|Too many login attempts(.*)/)
