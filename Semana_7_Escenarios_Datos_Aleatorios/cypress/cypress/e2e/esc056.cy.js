...
const NOMBRE_ESCENARIO = 'Escenario 056 -- Paso ';
const TAKE_SCREENSHOT = false;

describe("Login exitoso, crear pagina con titulo  y contenido de texto simple y luego editarla el titulo y contenido con direcciones de ethereum expuestos por una API", () => {
  beforeEach(() => {
    loginPage.visit();
    if(TAKE_SCREENSHOT) {
  ...
  it("Login y creación de un page y luego edicion", () => {
    ...
    cy.getEthereumAPI().then((data) => {
        // When I edit the page
        const content = data.ethereum;
