...
const NOMBRE_ESCENARIO = 'Escenario 041 -- Paso ';
const TAKE_SCREENSHOT = false;
const TWO_SECONDS = 2000;

describe("Login exitoso, crear pagina con titulo y contenido con solo texto aleatorio y luego editar el titulo con datos de texto de API", () => {
  beforeEach(() => {
    loginPage.visit();
    if(TAKE_SCREENSHOT) {
  ...
  it("Login y creación de un page", () => {
    ...
    cy.getBasicDataPageWithSimpleTextAPI().then((data) => {
        // When I edit the page
        crearPagina.editarPagina(initial_title, data.text);
        cy.wait(10000);
        if(TAKE_SCREENSHOT) { 
        ...
        crearPagina.revisarContenidoPagina(initial_title, data.text);
        cy.wait(10000);
        if(TAKE_SCREENSHOT) { 
