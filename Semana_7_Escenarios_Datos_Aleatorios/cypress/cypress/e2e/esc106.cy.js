...
const NOMBRE_ESCENARIO = 'Escenario 106 -- Paso ';
const TWO_SECONDS = 2000;

describe('Escenario 106', () => {
  beforeEach(() => {
    loginPage.visit();
    //cy.screenshot(NOMBRE_ESCENARIO + '0_visit');
  });

  it('Login con exito, creacion y validacion de un post con vimeo, signout', () => {
    ...
    cy.usePoliDinamicoCrearPost().then((formattedPost) => {
      crearPost.crearPostContenidoVimeo(formattedPost.title, formattedPost.title, formattedPost.body,5);
      cy.wait(TWO_SECONDS);
