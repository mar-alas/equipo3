...
const NOMBRE_ESCENARIO = 'Escenario 110 -- Paso ';
const TWO_SECONDS = 2000;

describe('Escenario 110', () => {
  beforeEach(() => {
    loginPage.visit();
    //cy.screenshot(NOMBRE_ESCENARIO + '0_visit');
  });

  it('Login con exito, creacion y validacion de un post con OTRA URL con url invalida', () => {
    ...
    cy.usePoliDinamicoCrearPost().then((formattedPost) => {
      crearPost.crearPostContenidoOtherUrl(formattedPost.title, formattedPost.title, formattedPost.body,5);
      cy.wait(TWO_SECONDS);
