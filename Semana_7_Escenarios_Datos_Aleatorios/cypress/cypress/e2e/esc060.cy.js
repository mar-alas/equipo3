...
const NOMBRE_ESCENARIO = 'Escenario 060 -- Paso ';
const TAKE_SCREENSHOT = false;

describe("Login exitoso, crear pagina con titulo  y contenido de texto simple y luego editar el titulo y contenido con cuentas de correo con claves expuestos por una API", () => {
  beforeEach(() => {
    loginPage.visit();
    if(TAKE_SCREENSHOT) {
  ...
  it("Login y creación de un page y luego edicion", () => {
    ...
    cy.getCredencialesAPI().then((data) => {
        // When I edit the page
        const content = data.email + "." +data.password;
